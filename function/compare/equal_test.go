...
func TestEqualFunc_Not_Nil(t *testing.T) {
	...
	neq := compare.EqualFunc[any](nil).Not()
	if neq != nil {
...
func TestEqualFunc_Reflexive_AnyEqual(t *testing.T) {
	...
	testEqualToReflexiveAnyEqual(t, req)
}
...
func TestEqualFunc_Reflexive_Nil(t *testing.T) {
	...
	req := compare.EqualFunc[any](nil).Reflexive()
	if req != nil {
		t.Error("got non-nil EqualFunc")
	}
}

func TestEqualToNot(t *testing.T) {
	t.Parallel()

	neq := compare.EqualToNot(compare.AnyEqual)
	testEqualToNotAnyEqual(t, neq)
}

func TestEqualToNot_Nil(t *testing.T) {
	t.Parallel()
	...
	neq := compare.EqualToNot[any](nil)
	if neq != nil {
		t.Error("got non-nil EqualFunc")
	}
}

func TestEqualToReflexive(t *testing.T) {
	t.Parallel()

	req := compare.EqualToReflexive(compare.AnyEqual)
	testEqualToReflexiveAnyEqual(t, req)
}

func TestEqualToReflexive_Nil(t *testing.T) {
	t.Parallel()
	...
	req := compare.EqualToReflexive[any](nil)
	if req != nil {
		t.Error("got non-nil EqualFunc")
	}
}

func testEqualToNotAnyEqual( //nolint:thelper // this is the main body, not a helper
	t *testing.T,
	neq compare.EqualFunc[any],
) {
	if neq == nil {
...
func testEqualToReflexiveAnyEqual( //nolint:thelper // this is the main body, not a helper
	t *testing.T,
	req compare.EqualFunc[any],
) {
	if req == nil {
		t.Error("got nil EqualFunc")
		return
...
func subtestEqual[T comparable]( //nolint:thelper // this is the main body, not a helper
	t *testing.T,
	name string,
	data []T,
) {
...
func subtestReflexiveEqual[T comparable]( //nolint:thelper // this is the main body, not a helper
	t *testing.T,
	name string,
	eqGroups [][]T,
) {
...
func subtestFloatEqual[T constraints.Float]( //nolint:thelper // this is the main body, not a helper
	t *testing.T,
	name string,
	eqGroups [][]T,
) {
...
var (
	...
	intsEqPairs             [][2][]int
	intsNeqPairs            [][2][]int
	float64sEqPairs         [][2][]float64
	float64sNeqPairs        [][2][]float64
	float64sWithNaNEqPairs  [][2][]float64
	float64sWithNaNNeqPairs [][2][]float64
	stringsEqPairs          [][2][]string
	stringsNeqPairs         [][2][]string
	anySliceEqPairs         [][2][]any
	anySliceNeqPairs        [][2][]any
)
...
func init() {
	...
	intsEqPairs, intsNeqPairs = mkEqNeqPairs(
		[][][]int{
			{nil},             // nil
			{{}},              // empty
	...
	float64sEqPairs, float64sNeqPairs = mkEqNeqPairs(
		append([][][]float64{
			{nil}, // nil
			{{}},  // empty
		}, float64sNonemptyEqGroups...),
		0,
		0,
	)
	...
	stringsEqPairs, stringsNeqPairs = mkEqNeqPairs(
		[][][]string{
			{nil},                       // nil
			{{}},                        // empty
			{{"1", "2", "3", "4"}},      // even length - 1
	...
	anySliceEqPairs, anySliceNeqPairs = mkEqNeqPairs(
		[][][]any{
			{nil},                  // nil
			{{}},                   // empty
			{{1, 2, 3, 4}},         // even length - 1
...
func subtestSliceEqual[T comparable]( //nolint:thelper // this is the main body, not a helper
	t *testing.T,
	name string,
	eqPairs [][2][]T,
	neqPairs [][2][]T,
...
func TestFloatSliceEqual(t *testing.T) {
	...
	for i := range float64sWithNaNEqPairs {
		for j := range float64sWithNaNEqPairs[i] {
			...
			f32EqPairs[i][j] = make(
				[]float32,
				len(float64sWithNaNEqPairs[i][j]),
			)
	...
	for i := range float64sWithNaNNeqPairs {
		for j := range float64sWithNaNNeqPairs[i] {
			...
			f32NeqPairs[i][j] = make(
				[]float32,
				len(float64sWithNaNNeqPairs[i][j]),
			)
	...
	subtestFloatSliceEqual(
		t,
		"type=[]float64",
		float64sWithNaNEqPairs,
		float64sWithNaNNeqPairs,
	)
}

func subtestFloatSliceEqual[T constraints.Float]( //nolint:thelper // this is the main body, not a helper
	t *testing.T,
	name string,
	eqPairs [][2][]T,
	neqPairs [][2][]T,
...
func subtestAnySliceEqual[T any]( //nolint:thelper // this is the main body, not a helper
	t *testing.T,
	name string,
	eqPairs [][2][]T,
	neqPairs [][2][]T,
...
func TestEqualToSliceEqual_FloatEqual_Float64(t *testing.T) {
	...
	for _, nilEqToEmpty := range []bool{false, true} {
		...
		subtestPairs(
			t,
			fmt.Sprintf("nilEqualsEmpty=%t", nilEqToEmpty),
			toSlice,
			eqPairs,
			neqPairs,
		)
...
func subtestSliceEqualWithoutOrder[T comparable]( //nolint:thelper // this is the main body, not a helper
	t *testing.T,
	name string,
	eqPairs [][2][]T,
	neqPairs [][2][]T,
...
func TestFloatSliceEqualWithoutOrder(t *testing.T) {
	...
	f64EqPairs, f64NeqPairs := mkEqNeqPairs(
		[][][]float64{
			{nil},
			{{}},
			{{1.}},
...
func subtestFloatSliceEqualWithoutOrder[T constraints.Float]( //nolint:thelper // this is the main body, not a helper
	t *testing.T,
	name string,
	eqPairs [][2][]T,
	neqPairs [][2][]T,
	...
	subtestPairs(
		t,
		name,
		compare.FloatSliceEqualWithoutOrder,
		eqPairs,
		neqPairs,
	)
}
...
var (
	...
	stringToIntEqPairs             [][2]map[string]int
	stringToIntNeqPairs            [][2]map[string]int
	stringToFloat64EqPairs         [][2]map[string]float64
	stringToFloat64NeqPairs        [][2]map[string]float64
	stringToFloat64WithNaNEqPairs  [][2]map[string]float64
	stringToFloat64WithNaNNeqPairs [][2]map[string]float64
	stringToStringEqPairs          [][2]map[string]string
	stringToStringNeqPairs         [][2]map[string]string
	stringToAnyEqPairs             [][2]map[string]any
	stringToAnyNeqPairs            [][2]map[string]any
)
...
func init() {
	...
	stringToIntEqPairs, stringToIntNeqPairs = mkEqNeqPairs(
		[][]map[string]int{
			{nil},
			{{}},
			{{"": 0}},
	...
	stringToAnyEqPairs, stringToAnyNeqPairs = mkEqNeqPairs(
		[][]map[string]any{
			{nil},
			{{}},
			{{"": nil}},
...
func TestMapEqual(t *testing.T) {
	...
	subtestMapEqual(
		...
		[][2]map[string]float64{
			...
			{
				{"A": floats.Inf64, "B": floats.NaN64A},
				{"A": floats.Inf64, "B": floats.NaN64A},
			},
			{
				{"A": floats.NaN64A, "B": floats.Inf64},
				{"A": floats.NaN64A, "B": floats.Inf64},
			},
		},
	)
...
func subtestMapEqual[K, V comparable]( //nolint:thelper // this is the main body, not a helper
	t *testing.T,
	name string,
	eqPairs [][2]map[K]V,
	neqPairs [][2]map[K]V,
...
func TestFloatValueMapEqual(t *testing.T) {
	...
	subtestFloatValueMapEqual(
		t,
		"type=map[string]float64",
		stringToFloat64WithNaNEqPairs,
		stringToFloat64WithNaNNeqPairs,
	)
}

func subtestFloatValueMapEqual[ //nolint:thelper // this is the main body, not a helper
	K comparable,
	V constraints.Float,
](
	t *testing.T,
...
func subtestAnyValueMapEqual[K comparable, V any]( //nolint:thelper // this is the main body, not a helper
	t *testing.T,
	name string,
	eqPairs [][2]map[K]V,
	neqPairs [][2]map[K]V,
...
func subtestPairs[T any]( //nolint:thelper // this is the main body, not a helper
	t *testing.T,
	name string,
